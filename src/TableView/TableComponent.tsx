    ...
    return (
        <Table dataSource={data} rowKey="key" size="small" bordered>
            ...
            <Column
                ...
                render={(tags: string[]) => (
                    tags && tags.length > 0 ? (
                        tags.map((tag) => <Tag color="geekblue" key={tag}>{tag.toUpperCase()}</Tag>)
                    ) : (
            ...
            <Column
                ...
                render={(record: FormValues) => (
                    <Space size="middle">
                        <a className='text-base' onClick={() => onEdit(record)}>Sửa</a>
                        <a className='text-base text-red-500' onClick={() => onDelete(record.key)}>Xóa</a>
                    </Space>
                )}
            />
        </Table>
